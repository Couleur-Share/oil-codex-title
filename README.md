...
```text
帮我查找 GitHub 仓库 Couleur-Share/oil-codex-title，下载并安装完整插件，开启话题自动命名并检查是否生效。需要我在界面中确认的步骤，请告诉我怎么操作。
```
...
目前为**预览版**，macOS 已实测，Windows CLI 已完成真实 Stop 命名验收，Linux 已通过自动化测试；Windows 桌面完整流程仍待实测，云端暂不支持。部分桌面版本的置顶列表可能仍显示旧标题，可让 Codex“检查真实标题并同步桌面显示”。
